...
static unsigned int active_port {20500};
...
TEST_F(LibModbusClientTest, testInitialization)
{
  ...
  server->startAsync(LOCALHOST, active_port);

  EXPECT_TRUE(client.init(LOCALHOST, active_port));
...
{
  LibModbusClient client;
  EXPECT_FALSE(client.init(LOCALHOST, active_port));
}
